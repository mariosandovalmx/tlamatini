...
#'
#' @return Boxplot de los contrastes posthoc en el modelo todas las variables explicativas del modelo.
#' @export
#'
#' @examples
#' library(tlamatini)
...
boxplot_emmeans <- function(formula, modelo, data, p.adjust.method=NULL){
  ...
  if(is.null(p.adjust.method)){
    ...
    bxp <- ggpubr::ggboxplot(data, x = var, y = respvar,fill = "gray", palette = "jco",bxp.errorbar=T,add = "jitter",add.params = list(color = "black", alpha=0.5)) +theme(legend.position = "none")
